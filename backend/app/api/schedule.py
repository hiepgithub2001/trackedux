...
async def get_past_sessions(
    ...
    lessons = await list_lessons(
        db, center_id=center_id, teacher_id=teacher_id, is_active=None
    )
    if not lessons:
    ...
    for lesson_obj in lessons:
        l_created = (
            lesson_obj.created_at.date()
            if hasattr(lesson_obj, "created_at") and lesson_obj.created_at
            else today
        )
    ...
    stmt = (
        base_query
        .join(Lesson, Lesson.id == LessonOccurrence.lesson_id)
        .order_by(
