import { $ as intersectRanges, A as binarySearch, B as formatDayString, C as ViewContainer, D as addDurations, E as addDays, F as buildSegTimeText, G as getSegMeta, H as formatIsoTimeString, I as computeEarliestSegStart, J as getUniqueDomId, K as getStickyFooterScrollbar, L as createDuration, M as buildEventRangeKey, N as buildIsoString, O as addMs, P as buildNavLinkAttrs, Q as injectStyles, R as createFormatter, S as StandardEvent, T as WeekNumberContainer, U as getEntrySpanEnd, V as formatIsoMonthStr, W as getSegAnchorAttrs, X as hasBgRendering, Y as groupIntersectingEntries, Z as hasCustomDayCellContent, _ as RefMap, a as ContentContainer, at as renderFill, b as Slicer, c as DayCellContainer, ct as sortEventSegs, d as DayTableModel, dt as _, et as intersectSpans, f as EventContainer, ft as d, g as PositionCache, h as NowTimer, i as BgEvent, it as rangeContainsMarker, j as buildEntryKey, k as asRoughMs, l as DayHeader, lt as startOfDay, m as NowIndicatorContainer, n as createPlugin, nt as memoize, ot as renderScrollShim, p as MoreLinkContainer, pt as y, q as getStickyHeaderDates, r as BaseComponent, rt as multiplyDuration, s as DateComponent, st as setRef, tt as isPropsEqual, u as DaySeriesModel, ut as wholeDivideDurations, v as SegHierarchy, w as ViewContextType, x as Splitter, y as SimpleScrollGrid, z as diffDays } from "./internal-CAkuLHK5.js";
...
//#endregion
//#region node_modules/@fullcalendar/timegrid/internal.js
...
function TimeColsAxisCell(props) {
	...
	return y(ViewContextType.Consumer, null, (context) => {
		if (!props.isLabeled) return y("td", {
			className: classNames.join(" "),
		...
		return y(ContentContainer, {
			...
			elAttrs: { "data-time": props.isoTimeStr },
			renderProps,
		...
		}, (InnerContent) => y("div", { className: "fc-timegrid-slot-label-frame fc-scrollgrid-shrink-frame" }, y(InnerContent, {
			elTag: "div",
...
var TimeBodyAxis = class extends BaseComponent {
	render() {
		return this.props.slatMetas.map((slatMeta) => y("tr", { key: slatMeta.key }, y(TimeColsAxisCell, Object.assign({}, slatMeta))));
	}
};
...
var TimeColsView = class extends DateComponent {
	constructor() {
		...
		this.renderHeadAxis = (rowKey, frameHeight = "") => {
			...
			let navLinkAttrs = diffDays(range.start, range.end) === 1 ? buildNavLinkAttrs(this.context, range.start, "week") : {};
			if (options.weekNumbers && rowKey === "day") return y(WeekNumberContainer, {
				elTag: "th",
				elClasses: ["fc-timegrid-axis", "fc-scrollgrid-shrink"],
				elAttrs: { "aria-hidden": true },
				date: range.start,
	...
	renderHScrollLayout(headerRowContent, allDayContent, timeContent, colCnt, dayMinWidth, slatMetas, slatCoords) {
		...
		sections.push({
			...
			chunks: [{
				key: "axis",
				content: (arg) => y("div", { className: "fc-timegrid-axis-chunk" }, y("table", {
					"aria-hidden": true,
					style: { height: arg.expandRows ? arg.clientHeight : "" }
				}, arg.tableColGroupNode, y("tbody", null, y(TimeBodyAxis, { slatMetas }))), y("div", { className: "fc-timegrid-now-indicator-container" }, y(NowTimer, { unit: isNowIndicator ? "minute" : "day" }, (nowDate) => {
					let nowIndicatorTop = isNowIndicator && slatCoords && slatCoords.safeComputeTop(nowDate);
					if (typeof nowIndicatorTop === "number") return y(NowIndicatorContainer, {
...
var TimeColsSlatsCoords = class {
	...
	computeDateTop(when, startOfDayDate) {
		if (!startOfDayDate) startOfDayDate = startOfDay(when);
	...
	render() {
		...
		return y("tbody", null, props.slatMetas.map((slatMeta, i) => {
			...
			return y("tr", {
				...
				elAttrs: { "data-time": slatMeta.isoTimeStr },
				renderProps,
...
var TimeColsSlats = class extends BaseComponent {
	...
	updateSizing() {
		...
		if (props.onCoords && props.clientWidth !== null) {
			if (this.rootElRef.current.offsetHeight) props.onCoords(new TimeColsSlatsCoords(new PositionCache(this.rootElRef.current, collectSlatEls(this.slatElRefs.currentMap, props.slatMetas), false, true), this.props.dateProfile, context.options.slotDuration));
		}
	}
};
...
function buildPositioning(segInputs, strictOrder, maxStackCnt) {
	...
	let hiddenGroups = groupIntersectingEntries(hierarchy.addSegs(segInputs));
	let web = buildWeb(hierarchy);
	web = stretchWeb(web, 1);
	return {
		segRects: webToRects(web),
		hiddenGroups
	};
}
function buildWeb(hierarchy) {
	...
	const buildNode = cacheable((level, lateral) => level + ":" + lateral, (level, lateral) => {
		let nextLevelRes = buildNodes(findNextLevelSegs(hierarchy, level, lateral), buildNode);
		let entry = entriesByLevel[level][lateral];
		return [Object.assign(Object.assign({}, entry), { nextLevelNodes: nextLevelRes[0] }), entry.thickness + nextLevelRes[1]];
	});
...
var TimeCol = class extends BaseComponent {
	...
	renderPositionedFgSegs(segs, segIsInvisible, isDragging, isResizing, isDateSelecting, forcedKey) {
		...
		let isMirror = isDragging || isResizing || isDateSelecting;
		let { segPlacements, hiddenGroups } = computeFgSegPlacements(segs, computeSegVCoords(segs, date, slatCoords, eventMinHeight), eventOrderStrict, eventMaxStack);
	...
	renderFillSegs(segs, fillType) {
		let { props, context } = this;
		return y(_, null, computeSegVCoords(segs, props.date, props.slatCoords, context.options.eventMinHeight).map((vcoords, i) => {
			let seg = segs[i];
			return y("div", {
				key: buildEventRangeKey(seg.eventRange),
				className: "fc-timegrid-bg-harness",
				style: computeSegVStyle(vcoords)
			}, fillType === "bg-event" ? y(BgEvent, Object.assign({ seg }, getSegMeta(seg, props.todayRange, props.nowDate))) : renderFill(fillType));
		}));
	}
	renderNowIndicator(segs) {
		...
		return segs.map((seg, i) => y(NowIndicatorContainer, {
			...
			elStyle: { top: slatCoords.computeDateTop(seg.start, date) },
			isAxis: false,
			date
		}));
	}
	computeSegHStyle(segHCoords) {
		...
		if (shouldOverlap && !segHCoords.stackForward) props[isRtl ? "marginLeft" : "marginRight"] = 20;
		return props;
	}
};
...
var TimeColsContent = class extends BaseComponent {
	...
	render() {
		...
		}, props.tableColGroupNode, y("tbody", { role: "presentation" }, y("tr", { role: "row" }, props.axis && y("td", {
			"aria-hidden": true,
			className: "fc-timegrid-col fc-timegrid-axis"
		}, y("div", { className: "fc-timegrid-col-frame" }, y("div", { className: "fc-timegrid-now-indicator-container" }, typeof nowIndicatorTop === "number" && y(NowIndicatorContainer, {
			elClasses: ["fc-timegrid-now-indicator-arrow"],
			elStyle: { top: nowIndicatorTop },
			isAxis: true,
			date: props.nowDate
	...
	updateCoords() {
		let { props } = this;
		if (props.onColCoords && props.clientWidth !== null) props.onColCoords(new PositionCache(this.rootElRef.current, collectCellEls(this.cellElRefs.currentMap, props.cells), true, false));
	}
};
